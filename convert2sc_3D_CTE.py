# -*- coding: utf-8 -*-
"""
Created on Tue Nov 01 16:54:42 2016
...
#abq_input='airfoil.inp'
#abq_input='weave2Dre2A.inp'
#new_filename='weave2Dre2B'
#macro_model=3
#specific_model=0
#analysis=0
#elem_flag=0
        ...
        if nmate != 0:
            for i in material.values():
                ...
                if i[2] == 0:  # isotropic
                    ...
                    if analysis==1:
                        try:
                            writeFormat(fout, 'EE', i[5:7] )
                        except:
                            print 'Isotropic CTE and specificHeat is not defined properly in material \'%s\' ' % repr(i[0])
                            print 'CTE must have the same type as the elastic properties: isotropic, orthotric or anithotropic for the same material.' 
                    ...
                    if analysis==1:
                        try:
                            writeFormat(fout, 'E'*4, i[12:16])
                        except:
                            print 'Orthotropic CTE and specificHeat is not properly defined in material \'%s\' ' % repr(i[0])
                            print 'CTE must have the same type as the elastic properties: isotropic, orthotric or anithotropic for the same material.' 
                elif i[2] == 2:
                    if len(i) == 12 and analysis ==0:   # orthotropic
                        ...
                        writeFormat(fout, 'E'*1, [                                 i[9]])
                        try:
                            writeFormat(fout, 'EEEEEEE', [i[12],i[13],i[14], 0.0, 0.0, 0.0, i[15]] )
                        except:
                            print 'Orthotropic CTE and specificHeat is not properly defined in material \'%s\' ' % repr(i[0])
                            print 'CTE must have the same type as the elastic properties: isotropic, orthotric or anithotropic for the same material.' 
                
                    elif len(i) == 24 and analysis ==0: # anisotropic
                    ...
                    if len(i) == 31 and analysis==1:
                        ...
                        try:
                            #  11   22  33  23  13   12
                            #  11   22  33  12  13   23
                            mp1=[i[24], i[25], i[26], i[29], i[28], i[27], i[30] ]
                            writeFormat(fout, 'EEEEEEE', mp1 )
                        except:
                            print 'Anisotropic CTE and specificHeat is not properly defined in material \'%s\' ' % repr(i[0])
                            print 'CTE must have the same type as the elastic properties: isotropic, orthotric or anithotropic for the same material.' 
                fout.write('\n')
            fout.write('\n')
